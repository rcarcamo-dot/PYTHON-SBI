...
The necessity for a program that is able to construct a full scale macrocomplex out of individual interacting pairs of proteins may seem redundant when presented in the style that we have here. After all we are simply deconstructing complexes found on PDB into their individual components and then building them back up again with their interacting pairs. In the real world there are many situations where you might be left with a plethora of information on what proteins are interacting at a pair level yet not know anything of the macrocomplex which these pairs are all a part of. In vivo experimental methods such as Flurescence Resonance Energy Transfer (FRET), Strep-protein interaction experiment (SPINE) as well as in vitro experimental methods such as co-immunoprecipitation and Tandem affinity Purification (TAP) can all gather information about two interacting proteins. One can easily see how data produced by experiments such  as these (provided that they then also produce some kind of pdb file assocated with the pair) would lend itself to a program as ours. Our protein compiler would be able to take that information and reconstruct any large scale macrocomplex that these individual pairs may be a part of.A severe limitation of our program however is that it relies on the fact that we need pdb files for the interacting protein. Obtaining 3d structures of proteins is a very complicated process and if you were looking to solve a problem from the ground up where you had to get the xyz coordinates for all of your interacting proteins you may as well give up. We decided to go with superpositioning because it was the most efficient way to reconstruct a macrocomplex when we know that we will have pdb files of all interacting proteins. In the real world we likely would never have all of these pdb files. Instead we may just have the sequences and the information that these proteins do interact. In this case it would be better to create the macrocomplex with programs such as PyDock that would be able to dock interacting pairs of proteins together in the right position before then superimposing them onto the same protein of another pair. 
## Tutorial
